<script lang="ts">
    ...
    async function fetchLeaderboardEntries() {
        isFetching = true;
        isFetching = true;
        const res = await fetch(`/api/leaderboards/${data.leaderboard.id}?page=${currentPage}`);
...
<AlertDialog.Root open={DeleteLeaderboardEntryDialogTrigger} onOpenChange={(open) => {
    ...
    <AlertDialog.Content>
        ...
        <AlertDialog.Footer>
            ...
            <form action="?/DeleteLeaderboardEntry" method="POST" use:DeleteLeaderboardEntryFormEnhance>
